...
export async function GET(req: NextRequest) {
  ...
  // Permission gate (documents.read) — preserved from the previous
  // implementation so existing access semantics are unchanged.
  {
    const { requirePermission } = await import("@/lib/permissions/can");
    const _d = requirePermission(auth, "documents.read");
    if (_d) return _d;
  }
